...
Dominus OS is a sovereign execution layer where AI operates as managed, auditable processes under named human authority. It provides a consistent structure for orchestrating agents and automated workflows under explicit authority controls, structured memory, and verifiable observability.

The objective is not more automation, but trustworthy automation: systems that remain accountable to human intent, resilient under stress, and auditable when outcomes matter.
...
The absence of a structured, governed cognitive substrate -- one that unifies execution, memory, authority, and observability -- is the next limiting factor in system evolution. The next abstraction layer is not another tool. It is a substrate.

## Core Invariants

1. **Authority is explicit** -- actions are bounded by roles, permissions, and approval gates; authority is structural, not behavioral
2. **Execution is observable** -- failures surface; work is measurable; outcomes are inspectable with unbroken attribution chains
3. **Side effects are controlled** -- writes and external actions pass through graduated approval gates; planning is unconstrained, execution is governed
4. **Memory is structured** -- short-term context and long-term knowledge are distinct, versioned, queryable, and attributable
5. **Operators remain sovereign** -- humans can halt, constrain, and review the system at any time; no automated process can outrun the operator's ability to intervene
...
- [Public Thesis White Paper (PDF)](docs/Dominus_OS_Public_Thesis_White_Paper_v0_3.pdf) -- the canonical thesis document
- [Architecture Overview](docs/ARCHITECTURE.md) -- conceptual architecture and component overview
...
> Lord, M. (2026). *Dominus OS: A Human-Governed Operating System for AI Workforces.* Dominus Foundry. https://dominusos.ai
