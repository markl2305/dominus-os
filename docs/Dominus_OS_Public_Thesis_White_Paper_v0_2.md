...
Dominus OS is not a concept paper. The following components are operational and serving real workloads:

- **Execution kernel.** A server-side orchestration kernel managing execution across hundreds of API endpoints, with real-time event distribution via WebSocket, structured job scheduling, and multi-tenant data isolation. The kernel routes objectives to virtual employees, enforces authority gates on every write operation, and maintains the canonical process ledger.
- **Virtual employees.** Multiple distinct virtual employees executing real work -- email management, CRM operations, document processing, operational intelligence, compliance enforcement, and cross-domain coordination -- each operating within governed authority envelopes that define what they can read, what they can modify, and what requires human approval. Each virtual employee has a published manifest describing its tools, authority level, and work surface.
- **Native operator applications.** A macOS desktop application built in Swift providing operator-level visibility into all running processes, tab-based task management for concurrent workflows, live WebSocket event streams, and direct halt authority over any active process. A companion iOS application extends operator visibility and intervention capability to mobile, including widget-based status monitoring.
- **Governed CRM.** A purpose-built web CRM application serving field teams with lead management, composite scoring, email attribution tracking, and signal intelligence. Unlike traditional CRMs, every record modification passes through the same governance kernel -- authority gates, audit trails, and attribution chains apply to CRM operations identically to all other system actions. The CRM includes automated lead scoring, follow-up enforcement, stale detection, and compliance truth-checking, all running as governed scheduled jobs.
- **Forge design system.** A unified design language -- typography, color tokens, spacing, and component patterns -- shared across the macOS and iOS operator applications. Forge ensures that the operator experience is consistent, accessible, and structurally tied to the governance layer rather than styled as an afterthought. Every interface element maps to a semantic token, enabling systematic updates across all operator surfaces.
- **Autonomous scheduled operations.** Dozens of scheduled jobs running under governance, each with defined authority levels, failure handling, and audit trails. These include CRM operations (lead scoring, truth checks, follow-up enforcement, stale detection), operational intelligence (briefing generation, performance analysis), and system maintenance tasks.
- **Immutable activity ledger.** A complete, append-only record of all system activity with full attribution chains linking every action to its authorization, its data sources, and its outcome.
- **Cross-domain signal intelligence.** A signal bus architecture that connects intelligence gathered in one business domain to decisions made in another, within tenant and authority boundaries. Signals detected in CRM (engagement patterns, sentiment shifts) surface in operational briefings. Signals from email analysis inform lead scoring. The intelligence crosses domain boundaries; the authority to act on it does not.
- **Integration layer.** Webhook-based integration with third-party services, subject to the same governance controls as internally initiated actions. GitHub repository operations, calendar management, and external API calls all pass through the same authority gates.
