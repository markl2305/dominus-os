...
**Added: March 2026** · **Revised: 23 September 2026** — the Formal Verification paragraph now names every check the TLC model checker runs — thirteen THEOREM properties and one well-formedness invariant, fourteen checks in all — and the bounds it runs within.
...
**Formal Verification.** Two TLA+ specifications of the governance architecture, `god-canon.tla` and `capability-model.tla`, declare thirteen THEOREM properties. The TLC model checker checks all of them, plus one well-formedness invariant, `TypeInvariant` in the capability model — fourteen checks in all, every one model-checked. Among the THEOREMs are the four this section has always named: (1) no sequence of syscalls can violate God Canon immutability (`CanonImmutability`, `CanonModificationImpossible`); (2) no capability derivation chain can produce escalated authority (`CapabilityMonotonicity`, with `TenantIsolation`); (3) no policy evaluation path can bypass God Canon checks (`CanonAlwaysChecked`); and (4) the kill switch is reachable from every possible system state (`KillSwitchLiveness`, `FreezeAlwaysReachable`). The other six are `EventLogAppendOnly`, `SuspensionRequiresAction`, `RevocationCascade`, `QuorumRequiresOperator`, `NoSelfAttestation` and `ActiveCapabilityInvariant`. TLC explores every reachable state of bounded models — at most two capabilities and four audit entries in the capability model, and four event-log entries in the God Canon model — and on 23 September 2026 it found no violation of any of the fourteen checks in 30,344 distinct states of the God Canon model and 3,861 of the capability model. A model check covers the models, not the running code: it shows the design holds these properties within those bounds, and it is not a proof that the deployed implementation does.
